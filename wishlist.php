...
if (!isset($user_id)) {
    ...
    exit(); // Stop the execution if user_id is not set
}
...
// Delete a single product from wishlist
if (isset($_GET['delete'])) {
    $delete_id = $_GET['delete'];

    // Ensure that delete_id is a valid number to prevent SQL injection
    if (is_numeric($delete_id)) {
        // Proceed with deletion if the ID is valid
        $delete_query = "DELETE FROM `wishlist` WHERE id = '$delete_id' AND user_id = '$user_id'"; // Add user_id to ensure only user's items are deleted
        $result = mysqli_query($conn, $delete_query) or die('query failed');
        header('location:wishlist.php'); // Redirect to wishlist page after deletion
        exit();
    } else {
        echo "Invalid ID!";
    }
...
if (isset($_GET['delete_all'])) {
    ...
    mysqli_query($conn, $delete_all_query) or die('query failed');
    header('location:wishlist.php');
    exit(); // Ensure that code execution stops after redirecting
}
...
<section class="wishlist">
    ...
    <div class="wishlist-total">
        ...
        <a href="wishlist.php?delete_all" class="delete-btn <?php echo ($grand_total > 0) ? '' : 'disabled' ?>"
